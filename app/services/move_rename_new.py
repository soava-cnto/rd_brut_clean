...
def create_name_from_file(file_path: str, retries=3):
    """Crée un nom de fichier et le nom du mois à partir du contenu d'un fichier Excel.
    
    Args:
        file_path: Chemin vers le fichier Excel
        retries: Nombre de tentatives en cas d'erreur de verrouillage
    """
    attempt = 0
    ...
    while attempt < retries:
        ...
        try:
            ...
            if attempt < retries:
                print(f"🔒 Fichier verrouillé : {os.path.basename(file_path)} - Tentative {attempt}/{retries}")
                time.sleep(2)  # Attendre avant de réessayer
            else:
                print(f"❌ Impossible de lire le fichier après {retries} tentatives : {os.path.basename(file_path)}")
                return None, None
